...
على جهاز واحد يظل مفتوحًا (جهاز الاستقبال مثلًا):

```bash
npm install
npm run build:server
npm run start:server           # يعمل على المنفذ 4321
```

ثم من أي جهاز على نفس الشبكة افتح `http://<عنوان-IP-للجهاز>:4321`.
...
`npm run build` ثم النشر كالمعتاد — يستخدم النظام تلقائيًا **Netlify Blobs** كقاعدة بيانات مشتركة بدل الملف المحلي، بنفس نظام الدخول والصلاحيات.
