...
يمكن **تجديد** المفتاح (فيتوقف القديم فورًا) أو **إيقاف الوصول** نهائيًا من نفس الشاشة.
...
**الأمان:** كلمات السر تُحفظ مُجزّأة بـ PBKDF2-SHA256 (210 آلاف دورة بملح عشوائي) ولا تُخزَّن كنص صريح أبدًا. رمز الجلسة يُحفظ مُجزّأً في كوكي `httpOnly` صالح 12 ساعة، وتُقفل المحاولات مؤقتًا بعد تكرار كلمة سر خاطئة، وتغيير كلمة سر مستخدم ينهي جلساته على كل الأجهزة. أما مفاتيح بوابة المريض (الرابط السري ورمز الدخول) فتُحفظ كنص لأن الاستقبال يحتاج إعادة إرسالها للمريض، وهي داخل قاعدة بيانات المرضى نفسها؛ ويُبطَل المفتاح بالتجديد أو الإيقاف فتنتهي جلسات المريض فورًا، وتُقفل محاولات الرمز الخاطئ مؤقتًا. جلسة المريض كوكي `httpOnly` منفصل تمامًا عن كوكي الموظفين، ولا يصل المريض إلا لبياناته هو بلا أي أسعار أو مدفوعات.
